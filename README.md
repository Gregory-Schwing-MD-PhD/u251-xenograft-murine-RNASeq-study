...
2. **Estimates W and re-runs DE.** `ANALYSIS/ruvseq_contamination_adjustment.R` runs `RUVs` at k = 1, 2, 3 and fits DESeq2 with `design = ~ W_1 + … + W_k + Classification` on the 6 Primary/Recurrent samples, alongside an unadjusted baseline (`~ Classification`). The DESeq2 step is **parameterized to match nf-core/differentialabundance v1.5.0** (same gene filter — count ≥ 10 in ≥ 1 contrast sample; Wald test, parametric fit, ratio size factors, `betaPrior = FALSE`, `independentFiltering = TRUE`), so the adjusted result differs from the differentialabundance baseline *only* by the added `W` covariate. (W is estimated over a separate stable expressed set, count ≥ 10 in ≥ 3 of the 9 samples.)
